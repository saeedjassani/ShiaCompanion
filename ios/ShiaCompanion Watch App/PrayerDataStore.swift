...
/// no direct equivalent for Flutter's cloud control points) that the cloud
/// here is rebuilt from circles rather than transliterated stroke-for-stroke.
/// The crescent, by contrast, is solved and sampled to match Flutter's path
/// exactly (see `crescent` below) rather than approximated. Everything else
/// — the ray angles, the horizon line, the star — maps straight across.
