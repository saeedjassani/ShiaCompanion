...
exports.buildZikrIndex = functions.firestore
    ...
    try {
        ...
        snapshot.docs.forEach((doc) => {
            ...
            const isCategory = doc.id.includes("~");
            const isAlias = doc.id.includes("|");
            if (!(hasData || isCategory || isAlias))
                return;
            // Skip alias if original has no data
            if (isAlias) {
                const originalId = doc.id.split("|")[1];
                const original = snapshot.docs.find((d) => d.id === originalId);
                if (original && !original.data().data?.toString().trim())
                    return;
            }
            if (data.title) {
                index[doc.id] = data.title;
            }
        });
